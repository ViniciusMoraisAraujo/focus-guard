...
// TestScheduler_SetInterceptorEnabled_SaveErrorRevertsRAM covers the same rollback
// family for the interceptor setting: when store.Save fails, the RAM value
// must revert to the persisted one instead of diverging from the disk until
// the next boot.
func TestScheduler_SetInterceptorEnabled_SaveErrorRevertsRAM(t *testing.T) {
	st, err := store.NewStore(filepath.Join(t.TempDir(), "state.json"))
	if err != nil {
		t.Fatalf("store: %v", err)
	}
	...
	// Estado persistido com sucesso: interceptor ligado.
	if err := sched.SetInterceptorEnabled(true); err != nil {
		t.Fatalf("SetInterceptorEnabled(true): %v", err)
	}
	if !sched.InterceptorEnabled() {
		t.Fatal("InterceptorEnabled should be true after successful set")
	}

	// Desligar falha no Save: a RAM deve reverter para true (valor persistido).
	failing.fail.Store(true)
	if err := sched.SetInterceptorEnabled(false); err == nil {
		t.Fatal("expected SetInterceptorEnabled to fail when store.Save fails")
	}
	if !sched.InterceptorEnabled() {
		t.Error("InterceptorEnabled should revert to the persisted value (true) after failed Save")
	}

	// Recuperação: com o disco de volta, a mudança funciona.
	failing.fail.Store(false)
	if err := sched.SetInterceptorEnabled(false); err != nil {
		t.Fatalf("SetInterceptorEnabled after recovery: %v", err)
	}
	if sched.InterceptorEnabled() {
		t.Error("InterceptorEnabled should be false after successful recovery set")
	}
}
