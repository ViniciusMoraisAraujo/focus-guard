...
// the *goal.Store satisfies it structurally. Handlers use package-local types;
...
type NoInput struct{}
type GetResult struct{ Goal time.Duration }
type SetInput struct{ GoalMinutes int }
...
func (h *GetHandler) Validate(*NoInput) error { return nil }

func (h *GetHandler) Handle(ctx context.Context, _ *NoInput) (*GetResult, error) {
	if h.store == nil {
		return nil, ipcerr.New(ipcerr.CodeNotConfigured, "meta diária não configurada")
	}
	return &GetResult{Goal: h.store.Get()}, nil
}
...
func (h *SetHandler) Handle(ctx context.Context, req *SetInput) (*SetResult, error) {
	if h.store == nil {
		return nil, ipcerr.New(ipcerr.CodeNotConfigured, "meta diária não configurada")
	}
	if req.GoalMinutes <= 0 || req.GoalMinutes > 24*60 {
		return nil, ipcerr.New(ipcerr.CodeInvalid, "meta inválida (entre 1 e 1440 minutos)")
	}
	...
	return &SetResult{
		Goal:    h.store.Get(),
		Message: fmt.Sprintf("Meta diária definida: %s", d.Round(time.Minute)),
	}, nil
}
