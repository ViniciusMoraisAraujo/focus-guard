...
// the same folder as the daemon's log (C:\Program Files\FocusGuard on
// Windows, /opt/focusguard on Linux).
const logFileName = "focusguard-watchdog.log"
...
// setupLogging wires the watchdog's log to the install directory (next to the
...
func setupLogging() func() {
	...
	if err != nil {
		log.Printf("[FocusGuard Watchdog] Log em arquivo indisponível (%s): %v", path, err)
		return func() {}
	}
	return restore
}
